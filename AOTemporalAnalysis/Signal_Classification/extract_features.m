function [ feature_coeffs, labels ] = extract_features( cell_times, norm_cell_reflectance, critical_period, vid_type )
    ...
    feature_coeffs=zeros(length(norm_cell_reflectance),23);
...
%         poststim = find(times>critical_period(1));
        ...
        interpsignal = interpsignal(1:maxind);
        
        nextpowpad = (2^nextpow2(length(interpsignal)))-length(interpsignal);
        padsignal = padarray(interpsignal, [0 nextpowpad],'symmetric','pre');
        ...
        SWC = SWC(:,poststim(1)+nextpowpad:end);
...
%         DWC = detcoef(c,l,1:level);
        DWC = modwt(interpsignal,wavelet,4);
...
%         plot(denoised_signal); hold on; plot(interpsignal);       
%         stim_affinity
%         auc
