function []=FF_Temporal_Reflectivity_Analysis(mov_path, ref_image_fname, stimulus_frames, vid_type, secondsection)
...
if ~exist('mov_path','var') || ~exist('ref_image_fname','var')
    ...
    rply = input('Stimulus (s), Control (c), or Interleaved (i)? [i] ','s');
    if isempty(rply) || strcmpi(rply,'i')
        vid_type='interleaved';
        
        rply = input('When does the second section start? ','s');
        if ~isnan(str2double(rply))
            secondsection = str2double(rply);
        else
            error('Must be a number.');
        end 
    elseif strcmpi(rply,'c')
        vid_type='control';
    elseif strcmpi(rply,'s')
        vid_type='stimulus';
    end
end
if ~exist('secondsection','var')
    secondsection=[];
end
...
while(hasFrame(temporal_stack_reader))
    temporal_data(:,:,i) = double(readFrame(temporal_stack_reader));
    i = i+1;
end
...
if ~isempty(secondsection)
    section_loc = find(stimd_images>=secondsection);
    section_loc = section_loc(1);
    section_times = [1 section_loc-1;
                     section_loc size(temporal_data,3)];
else
...
for s=1:size(section_times,2)
    ...
    if ~isempty(stim_inds)
        capillary_mask = double(~tam_etal_capillary_func( temporal_stack(:,:,1:stim_inds(1)-1) ));
    else
    ...
    ref_mean=zeros(1,size(cell_ref,2));
    ref_hist=[];
    ref_std=zeros(1,size(cell_ref,2));
