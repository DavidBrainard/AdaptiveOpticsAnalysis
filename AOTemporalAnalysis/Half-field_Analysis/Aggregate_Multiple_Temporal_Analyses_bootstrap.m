...
% [fitCharacteristics]=Aggregate_Multiple_Temporal_Analyses_bootstrap(rootDir)
%
%   Calculates pooled variance across a set of pre-analyzed 
%   signals from stimulus and control trials, performs the subtraction
%   between between randomly selected trials' standard deviations,
%   and performs a piecewise fit of the resulting subtraction.
%
%   Normally, the user doesn't need to select a root
%   directory (that will be found automatically by
%   "aggregate_multi_trial_run.m"), but if the software is run by
%   itself it will prompt the user for the folder containing the
%   pre-analyzed mat files generated by Temporal_Reflectivity_Analysis.m.
%
% Inputs:
%       rootDir: The folder path of the pre-analyzed (.mat) trials. Each
%       mat file must contain valid stimulus and control signals.
%
% Outputs:
